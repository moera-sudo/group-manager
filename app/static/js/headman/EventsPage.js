document.addEventListener('DOMContentLoaded', function() {
    ...
    eventsSection.addEventListener('click', async function(e) {
        ...
        if (confirm('Вы уверены, что хотите удалить это событие?')) {
            try {
                const response = await fetch(`/headman/account_main/${currentUserId}/events/${eventId}/delete`, {
                    method: 'POST'
                });
    ...
    eventForm.addEventListener('submit', async function(e) {
        ...
        try {
            const response = await fetch(`/headman/account_main/${currentUserId}/events`, {
                method: 'POST',
                body: formData
            });
