document.addEventListener('DOMContentLoaded', () => {
    ...
    //обязательно оставить(этот скрип отвечает за анимацию 
    ...
    voteSection.addEventListener('click', function(e) {
        ...
        // Этот код нужно будет доработать с получением существующих вариантов ответов
        // Пока что просто добавляем одно текстовое поле
    ...
    voteSection.addEventListener('click', async function(e){
        ...
        if (confirm('Вы уверены, что хотите удалить этот опрос?')){
            try {
                const response = await fetch(`/admin/account_main/${currentUserId}/votes/${voteId}/delete`, {
                    method: 'POST'
                });
    ...
    voteSection.addEventListener('click', async function (e){
        ...
        if (confirm(confirmMessage)){
            try {
                const response = await fetch(`/admin/account_main/${currentUserId}/votes/${voteId}/change`,{
                    method: 'POST'
                });
